...
#
# Writes: dist/fighting-force-recomp-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix fighting-force-recomp \
  --exe-name Fighting_Force__Recompiled \
  --display-name "Fighting Force Recompiled" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --disc-hint "your legally owned Fighting Force disc" \
